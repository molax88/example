 ...
 * 调用方式如下
 * <input class="Wdate Wdate-1" type="text" value="" name="date"/>
 * 一，WdatePicker({readOnly:false,minDate:'2015-05-01',maxDate:'2015-10-30',dateFmt:'yyyy-MM-dd HH:mm:ss',alwaysUseStartDate:true});
 * 二，WdatePicker({minDate:'%y-%M-%d'}) '%y-%M-%d'表示当前日期
 * 三，WdatePicker()
 * 四,WdatePicker({minDate:'#F{$dp.$D(\'上一个wdatePick的input框ID名，没有#号\',{d:0});}')限制最小日期为上一个input的日期,
 * 		{d:0}表示将该日期限制增加或减少相应的天数为0,可以改变值
